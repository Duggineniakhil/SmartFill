  ...
  for (const [k, v] of entries) {
    ...
    del.onclick = () => {
      chrome.storage.local.get({ profile: {} }, (d) => {
        delete d.profile[k];
        chrome.storage.local.set({ profile: d.profile }, load);
      });
    };
    row.appendChild(del);
...
function load() {
  chrome.storage.local.get({ profile: {}, enabled: true, autoFill: false }, (d) => {
...
$("addBtn").addEventListener("click", () => {
  ...
  chrome.storage.local.get({ profile: {} }, (d) => {
    d.profile[k] = v;
    chrome.storage.local.set({ profile: d.profile }, () => { $("newVal").value = ""; load(); });
  });
});

$("clear").addEventListener("click", () => {
  if (!confirm("Delete all stored profile data?")) return;
  chrome.storage.local.set({ profile: {} }, load);
});

$("fillNow").addEventListener("click", async () => {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (!tab?.id) return;
  chrome.tabs.sendMessage(tab.id, { type: "AUTOFLOW_FILL_NOW" }, (res) => {
    const btn = $("fillNow");
